...
for _ci, _sc in enumerate(_active):
    ...
    with _mc_cols[_ci]:
        ...
        if _agg is None or _agg.empty:
            ...
            if _spend_t is not None: st.metric("Total Spend @ State/Month Level Only", _fmt_spend(_spend_t))
            if _cpf     is not None: st.metric("CPF @ State/Month Level Only",         _fmt_cpf(_cpf))
