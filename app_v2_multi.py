...
if _active_scens:
    ...
    if _full_monthlys:
        ...
        _view_label = _view_col.radio("APPS View", ["All", "No-Spend", "Incremental"], horizontal=True, key="monthly_apps_view")
        ...
        if m_display is None or m_display.empty:
            ...
            _has_orig_data = _origination_col in m_display.columns and _appr_apps_sum > 0
            _blended_appr_rate = m_display[_approval_col].sum()    / _appr_apps_sum if _has_appr_data else 0.0
            _blended_orig_rate = m_display[_origination_col].sum() / _appr_apps_sum if _has_orig_data else 0.0
            ...
            _n_mc  = 1 + (1 if _appr_total is not None else 0) + (1 if _orig_total is not None else 0)
            ...
            if _orig_total is not None:
                _orig_col_idx = 1 + (1 if _appr_total is not None else 0)
                _mcols[_orig_col_idx].metric("Likely Originations", f"{_orig_total:,}")
                if _has_orig_data:
                    _mcols[_orig_col_idx].markdown(
                        f"<div style='font-size:0.75rem;color:var(--text-color);opacity:0.6;margin-top:0.15rem'>"
                        f"Origination Rate: <strong>{_blended_orig_rate * 100:.0f}%</strong></div>",
                        unsafe_allow_html=True,
                    )

            _monthly_primary_cols = ["State", "Period"]
            _col_rename = {}
            ...
            _monthly_fmt   = {c: "{:,}" for c in _monthly_primary_cols if c not in ["State", "Period"]}
